...
A normal distribution is the familiar "bell curve" and it's a way of formalizing a distribution where observations cluster around some central tendency. Observations farther from the central tendency occur less frequently (@fig-dnormal).
...
In an approximately **bell-shaped** distribution:
...
The z is a random variable that has a Standard Normal distribution, also called a z-distribution, i.e. a special normal distribution where $\mu=0$ and $\sigma^2=1$. In this case, @eq-gauss is transformed as follows:
...
**Fifth**, if we want to find the area E between $z_1$ = 1 and $z_2$ = 2, we must integrate the @eq-zd as following:
...
::: {.callout-tip icon="false"}
## Definitionnd properties of t-distribution
...
$$\chi^2 =\sum _{i=1}^{n}Z_{i}^{2}$$ {#eq-chi}
...
**Basic Properties of chi-squared distribution:**
The chi-squared distribution is always positive and its shape is uniquely determined by the degrees of freedom. The distribution becomes more symmetrical as the degrees of freedom increase and when df>50, the chi-squared distribution is very similar to the Normal distribution.
...
Let $\chi_n^2$ and $\chi_m^2$ be independent variates distributed as chi-squared with n and m degrees of freedom.
$$F_{n,m}=\dfrac{X_n/n}{X_m/m}$$
...
curve(df(x, df1 = 2, df2 = 1), from = 0, to = 5, col= 'black', lwd=2, xlab = "F", ylab = "Probability Density")
curve(df(x, df1 = 4, df2 = 6), from = 0, to = 5, col= 'red', lwd=2,
      add = TRUE)
curve(df(x, df1 = 12, df2 = 12), from = 0, to = 5, col= 'green', lwd=2,
      add = TRUE)
