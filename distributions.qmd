...
where $e \approx 2.718$ and $\pi \approx 3.14$
...
(a) Between what values of  x  do 68% of the values lie?

The X random variable of sbp follows the normal distribution:

$$ X ∼ N(125, 15^2)$$

We know that 68% of the distribution is between -1 $\sigma$ below the mean and +1 $\sigma$ above the mean:
...
The Z is a random variable that has a Standard Normal distribution, also called a z-distribution, i.e. a special normal distribution where $\mu=0$ and $\sigma^2=1$. 

$$ Z ∼ N(0, 1)$$

In this case, @eq-gauss is transformed as follows:
...
Calculate the orange area under the curve:
...
plot(x, y, main="Area under the curve", frame.plot = FALSE,
...
### **Statistical moments (or shape statistics) and Normal distribution**
...
If the shape of empirical distributions of random variables approximates the Gaussian distribution then the variables are considered to be distributed normally. One such example is the histogram of the birthweight (in kg) of 3226 new-born babies (reference).
...
plot(h, freq=TRUE, main = "Empirical distribution ", xlab = "Birthweight (in kg)", ylab="Relative Frequency")
...
qqline(data_birthweight$birthweight, col = "steelblue",lty = 2, lwd = 2)
```
