...
This textbook includes **two** Parts, **"Medical Statistics"** and **"Jamovi LAB"**. For each theoretical chapter (e.g., "Descriptive Statistics") in first part there is a practical lab with Jamovi in the second part ("Lab II:Descriptive Statistics").

Additionally, throughout this textbook there are some inserts or callouts (colored "boxes") that are designed to bring out some of the important information and help our learning. These inserts are distinguished by different colors:
...
$\color{orange}{Orange\ color\ with\ a\ traffic\ cone }$: Additional information that can be expanded by the user (click on it).
...
$\color{red}{Red\ color\ with\ an\ exclamation\ mark}$: Important point we should pay attention to.
