...
#| warning: false
#| label: fig-histo1
#| fig-cap: Distribution of age of the participants in @farndon2013 study (a) histogram (b) density plot.
#| fig-width: 7.5
#| fig-height: 5.0
...
#| warning: false
#| label: fig-histo2
#| fig-cap: Histogram of age for different bin sizes (a) bin size = 5, (b) bin size = 10, and (c) bin size = 15.
#| fig-width: 8.5
#| fig-height: 4.5
...
p1 <- dat_corn |>
  ggplot(aes(x = age)) +
  geom_histogram(binwidth=5, fill="steelblue4", color="#8fb4d9", alpha=0.6) +
  ggtitle("Bin size = 5") +
  labs(y="Frequency") +
  theme_minimal() +
  theme(
...
A box plot chart is another graph that can be used for conveying location and variation information for continuous data, particularly for detecting
changes between different groups of data before any formal analyses are performed.
...
There are many variations of the boxplot. For example, there is a way to combine raw data (dots), probability density, and key summary statistics such as median, mean, and relevant confidence intervals in an appealing and flexible format with minimal redundancy, using the raincloud plot:
...
#| warning: false
#| label: fig-boxplot4
#| fig-cap: A rain cloud plot of age of the participants in @farndon2013 study.
#| fig-width: 6.5
#| fig-height: 4.0
